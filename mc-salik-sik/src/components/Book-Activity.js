...
const BookActivityTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/bookactivities');
  ...
  const handleAction = async (activityId, actionType) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        const response = await fetch(`http://localhost:3000/api/bookactivity_reserve/${activityId}`, {
          method: 'PUT',
          headers: {
        ...
        const updatedResponse = await fetch('http://localhost:3000/api/bookactivities');
        const updatedData = await updatedResponse.json();
        setBookActivities(updatedData);
        setFilteredActivities(updatedData);
  
        // Show success after loading has been closed
        Swal.fire({
          icon: 'success',
          title: 'Success!',
            ...
            {filteredActivities.length > 0 ? (
              filteredActivities.map((activity) => (
                <TableRow key={activity.activity_id} hover>
                  <TableCell style={{ maxWidth: '240px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{activity.title}</TableCell>
                  <TableCell style={{ textAlign: 'center' }}>{activity.stocks !== undefined ? activity.stocks : 'N/A'}</TableCell> {/* Check for undefined */}
                  <TableCell style={{ maxWidth: '150px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{activity.user_email}</TableCell>
