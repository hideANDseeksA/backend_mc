...
const BookLogs = () => {
  ...
  useEffect(() => {
    const fetchBookLogs = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get('http://localhost:3000/api/books_history'); // Replace with your API endpoint
  ...
  const deleteAllBookActivities = async () => {
    try {
      const response = await fetch('http://localhost:3000/api/books_history', {
        method: 'DELETE',
      });
