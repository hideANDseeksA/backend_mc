...
const StudentDashboard = () => {
  ...
  const handleAction = async (email, action, value) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        await axios.put(`http://localhost:3000/api/students/${email}/${action}`, { [action]: value });
        fetchStudents();
  ...
  const handleInsertNewStudents = async () => {
    ...
    if (confirmed.isConfirmed) {
      ...
      try {
        const response = await axios.post("http://localhost:3000/api/insert_students", {
  ...
  const handleVerifyAll = async (enrolled) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        await axios.put(`http://localhost:3000/api/students/verifyAll`, { enrolled });
        fetchStudents();
